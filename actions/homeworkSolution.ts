...
export const uploadSolution = withResult(
  async (
    participantId: number,
    formData: FormData,
  ) => {
    const currentUser = await user();
    if (!currentUser || !currentUser.studentId) return fail("غير مصرح");
    ...
    if (!participant) return fail("المشاركة غير موجودة");
    if (participant.studentId !== currentUser.studentId)
      return fail("غير مصرح لك برفع حل لهذه الحصة");

    if (!participant.session.assignment)
      return fail("لا يوجد واجب لهذه الحصة");
    ...
    if (existing)
      return fail("لقد قمت برفع حل مسبقاً. احذفه أولاً أو عدّله.");
...
export const gradeSolution = withResult(
  async (
    ...
    if (score < 0 || score > (solution.assignment.maxScore || 10)) {
      return fail("الدرجة خارج النطاق المسموح");
    }
...
export const deleteSolution = withResult(async (solutionId: number) => {
  const currentUser = await user();
  if (!currentUser) return fail("غير مصرح");
  ...
  if (!isOwner && !isTutor && !isAdmin) return fail("غير مصرح");
