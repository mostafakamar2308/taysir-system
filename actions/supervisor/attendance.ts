...
export async function markStudentAttendanceBySupervisor(
...
export async function upsertTutorAttendance(
  sessionId: number,
  status: AttendanceStatus,
  notes?: string,
) {
  ...
  if (getSessionStatus(session) !== SessionStatus.COMPLETED) {
    throw new Error("لا يمكن تقييم حضور المعلم إلا بعد انتهاء الحصة");
  }
